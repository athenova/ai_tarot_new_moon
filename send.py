...
if __name__ == '__main__':
    ...
    if new_moon_date == check_date:
        ...
        schedule.every().day.at("20:32",'Europe/Moscow').do(job, type="taurus")
        schedule.every().day.at("20:33",'Europe/Moscow').do(job, type="gemini", chat_id='@gemini_the', group_id='229837895')
        schedule.every().day.at("20:34",'Europe/Moscow').do(job, type="cancer")
        schedule.every().day.at("20:35",'Europe/Moscow').do(job, type="leo")
        schedule.every().day.at("20:36",'Europe/Moscow').do(job, type="virgo")
        schedule.every().day.at("20:37",'Europe/Moscow').do(job, type="libra")
        schedule.every().day.at("20:38",'Europe/Moscow').do(job, type="scorpio")
        schedule.every().day.at("20:39",'Europe/Moscow').do(job, type="sagittarius")
